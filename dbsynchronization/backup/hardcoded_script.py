...
def main():
    ...
    for idx, table_info in enumerate(target_config['tables']):
        ...
        print(f"Processing mapping for Reference Table '{reference_table}' and Target Table '{target_table}'...")

        reference_data = fetch_db_data(reference_conn, reference_table, reference_columns, reference_db_type)
        target_data = fetch_db_data(target_conn, target_table, target_columns, target_db_type)

        insert_commands, update_commands, insert_mysql_commands = synchronize_tables(
            reference_data, target_data, reference_key_column, target_key_column, reference_table, target_table,
            key_column_mapping, reference_conn, target_conn, fk_mappings, reference_table_info, target_table_info)

        data_differences(reference_data, target_data, reference_key_column, target_key_column, key_column_mapping)
