from DBSync.fetch_indirect_key import fetch_indirect_key


def synchronize_tables(reference_data, target_data, reference_key_column, target_key_column, reference_table, target_table,
                       key_column_mapping, mysql_conn, postgres_conn, fk_mappings, reference_table_info, target_table_info):
    insert_sql_commands = []
    update_sql_commands = []
    ...
    for reference_id, reference_row in reference_dict.items():
        ...
        if reference_id in target_dict:
            ...
            for mapping in key_column_mapping:
                ...
                if reference_row[reference_column] != target_row.get(target_column, None):
            ...
            for fk_mapping in fk_mappings:
                ...
                if fk_mapping['dependant_table'] == target_table:
                    ...
                    dependant_value = fetch_indirect_key(postgres_conn, reference_row, fk_mappings, dependant_table)
