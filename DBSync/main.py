...
from auto_synchronize import auto_synchronize
from data_differences import data_differences
...
def main():
    ...
    for mapping in mappings_config["mappings"]:
        ...
        reference_table_info = next(
            (
                table
                for table in source_config["tables"]
                if table["name"] == reference_table_name
            ),
            None,
        )
        ...
        target_table_info = next(
            (
                table
                for table in target_config["tables"]
        ...
        print(
            f"Processing mapping for Reference Table '{reference_table_name}' and Target Table '{target_table_name}'..."
        )

        reference_data = fetch_db_data(
            reference_conn,
            reference_table_info["name"],
            reference_table_info["columns"],
            reference_db_type,
        )
        target_data = fetch_db_data(
            target_conn,
            target_table_info["name"],
            target_table_info["columns"],
            target_db_type,
        )

        insert_commands, update_commands, insert_reference_commands = (
            synchronize_tables(
                reference_data,
                target_data,
                reference_table_info["key_column"],
                target_table_info["key_column"],
